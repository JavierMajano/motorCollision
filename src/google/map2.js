...
class App extends Component {
  ...
  currentBorough = (event) => {  // handles buttom click and this is used for getting the selected borough & creating the boundary
    ...
    coordarr = [];
    for (let x = 0; x < this.state.polyShapes.length; x++) {
      if (this.state.polyShapes[x] !== undefined) {
        this.state.polyShapes[x].setMap(null)
      }

    }
    for (let x = 0; x < this.state.circleShapes.length; x++) {
      if (this.state.circleShapes[x] !== undefined) {
        this.state.circleShapes[x].setMap(null)
      }
    }
    ...
    fetch(url, options)
      ...
      .then(json => {
        ...
        for (let x = 0; x < json.length; x++) {
          ...
          newCircles.push(multiCircle)
          multiCircle.setMap(map)
        }
        this.setState({circleShapes: newCircles })
      }
      )
    ...
    for (let j = 0; j < motorData.features.length; j++) {
      ...
      if (motorData.features[j].properties.BoroName === this.state.selectedOption.value) {
        ...
        for (let i = 0; i < geom.length; i++) {
              if(motorData.features[j].geometry.type === 'MultiPolygon')
              {
                console.log(motorData.features[j].geometry.type)
                for(let k = 0; k< geom[i].length; k++)
                {
                  console.log(geom[i][k])
                  coordarr2 = geom[i][k].map(coord => ({ lat: coord[1], lng: coord[0] }));
                }
    ...
    this.setState({ polyShapes: newPolygons})
    //console.log(coordarr2)
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <Map />
        <div className='col-sm'> {boroug.map((content, i) => <Con crashes={content.properties.Crashes} hood={content.properties.NTAName} keys={content.key} />)}</div>
      </div>
    );
  }
}
